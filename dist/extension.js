...
Global.ChatGPT_KEY = "fengzheng_chatgpt_api_key";
...
const constant_1 = __webpack_require__(43);
const stream_1 = __webpack_require__(7);
class BitGPTViewProvider {
    constructor(_extensionUri, context) {
        this._extensionUri = _extensionUri;
        this.chatContext = [];
        this.isFirst = true;
        this.htmlKey = 'fengzheng.bitGPT.html';
        this.saveHtml = (html) => {
            this._context.workspaceState.update(this.htmlKey, html);
        };
        this.getHtml = () => {
            const containerHtml = this._context.workspaceState.get(this.htmlKey) || '';
            let html = this.getContent();
            html = html.replace(`<div id="container">`, `<div id="container">${containerHtml}`).replace(`class="write"`, `class=""`);
            return html;
        };
        this.getApiKey = () => {
            return this._context.globalState.get(constant_1.Global.ChatGPT_KEY) || '';
        };
        this.checkApiKey = () => {
            ...
            if (key === '') {
                vscode.window.showInputBox({
                    ignoreFocusOut: true,
                    password: false,
                    prompt: "please enter ChatGPT API key",
                    value: '' // 默认值, 默认全部选中
                }).then(value => {
                    if (!value || !value?.trim()) {
                        vscode.window.showErrorMessage("The key you entered is invalid");
                        return;
                    }
                    ;
                    this._context.globalState.update(constant_1.Global.ChatGPT_KEY, value);
                });
            }
        };
        this.setLastAnwser = (context) => {
            if (this.chatContext.length === 2) {
                this.chatContext.shift(); // 移除第一个元素
            }
            this.chatContext.push(context);
        };
    ...
    resolveWebviewView(webviewView, context) {
        ...
        if (this.isFirst) {
            webviewView.webview.html = this.getContent();
            this.isFirst = false;
        }
        this.checkApiKey();
        webviewView.webview.onDidReceiveMessage(message => {
            switch (message.command) {
                case 'alert':
                    this.getDataFromHttps(message.text, message.model)
                        .then(data => {
                        const result = JSON.stringify(data);
                        // console.log(`获取到的数据为：${result}`);
                        // vscode.window.showErrorMessage(`${result}`);
                    ...
                    return;
                case 'giveLastAnswer':
                    this.setLastAnwser(message.text);
                    this.saveHtml(message.html);
                    return;
            }
        }, undefined);
    ...
    //let model = "gpt-4";
    async getDataFromHttps(prompt, model) {
        const url = 'https://api.openai.com/v1/chat/completions';
        console.log("model = " + model);
        const webview = this._view?.webview;
        var processingQuestion = `Questions and answers from the first two rounds:\`\`\``;
        if (this.chatContext != null) {
            this.chatContext.forEach(item => {
                processingQuestion += item;
            });
        }
        processingQuestion += `\`\`\` Q："${prompt}"`;
        let question = {
            ...
            "model": model,
            "messages": [{ "role": "user", "content": processingQuestion },
                { "role": "system", "content": "The beginning of the answer does not need to add A：" }],
            "stream": true
        };
        let postData = JSON.stringify(question);
...
exports.BitGPTViewProvider = BitGPTViewProvider;
...
function activate(context) {
    ...
    const provider = new sidebarWebview_1.BitGPTViewProvider(context.extensionUri, context);
    context.subscriptions.push(vscode.window.registerWebviewViewProvider(sidebarWebview_1.BitGPTViewProvider.viewType, provider));
    let disposable = vscode.commands.registerCommand('fengzheng.chatgpt', () => {
        ...
        if (apiKey.trim() === '') {
            vscode.window.showInputBox({
                ...
                ;
                context.globalState.update(constant_1.Global.ChatGPT_KEY, value);
                (0, chat_1.default)(context);
            });
        }
        else {
            (0, chat_1.default)(context);
     ...
     * Editor 操作
     */
    (0, eidorAction_1.default)(context);
}
